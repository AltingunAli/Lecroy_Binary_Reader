...
void MyFunctions() { cout << "Loading functions library..." << endl; }
...
void RegisterRunParameters(RUNPAR *runpar, const char *dirname) {
  const char title[18][12] = {
      "detNo",    "runNo",     "ampl",  "sampl",  "charge",   "scharge",
      "rate",     "srate",     "grate", "sgrate", "tot",      "stot",
      "risetime", "srisetime", "width", "swidth", "chovampl", "schovampl"};
  ...
  } else /// read existing table entries
  {
    ...
    cout << endl
         << "Found " << entries << " existing entries in the table file "
         << filename << endl
         << endl;
    inf.close();
  }
  ...
  for (int i = 2; i < 18; i++)
    fprintf(outf, "%14s ", title[i]);
  fprintf(outf, "\n");
  for (int i = 0; i < TOTRUNS * 100; i++) {
    //      irun = allruns[i].runNo - MINRUN;
    irun = i;
    if (allruns[irun].detNo <= 0 || allruns[irun].runNo < 0)
      continue;
    //      cout<<"Writing entry "<<irun<<" corresponding to "<<
    //      allruns[irun].runNo<<endl;
    fprintf(outf, "%6d %6d %14e %14e %14e %14e %14e %14e %14e %14e ",
            allruns[irun].detNo, allruns[irun].runNo, allruns[irun].ampl,
            allruns[irun].sampl, allruns[irun].charge, allruns[irun].scharge,
            allruns[irun].rate, allruns[irun].srate, allruns[irun].grate,
            allruns[irun].sgrate);
    fprintf(outf, "%14e %14e %14e %14e %14e %14e %14e %14e\n",
            allruns[irun].tot, allruns[irun].stot, allruns[irun].risetime,
            allruns[irun].srisetime, allruns[irun].width, allruns[irun].swidth,
            allruns[irun].chovampl, allruns[irun].schovampl);
  }
  fclose(outf);
}
...
float calculateTime(double Rstart, double Rend, int SCpoints, double *lt,
                    int *Condition) {
  float time = 0.;
...
float calculateBGTime(double Rstart, double Rend, int SCpoints, double *lt,
                      int *Condition, int *Tracking) {
  float time = 0.;
...
int GetProjectionFitParameters(TH2D *histo2d, double *mean, double *sigma,
                               double *gmean, double *gsigma, double *runs,
                               int maxr) {
  TH1D *proj;
  //     TH1D* proj2;
...
void DrawGraphsFrom(TH2D *pthc, TH2D *pth, int maxr, char *txt1, char *txt2) {
  ...
  double bmean[10000], bsigma[10000], bgmean[10000], bgsigma[10000],
      bfitruns[10000];
  double err0[10000];
  char txt[1000];

  int ntst = GetProjectionFitParameters(pthc, mean, sigma, gmean, gsigma,
                                        fitruns, maxr);
  int bntst = GetProjectionFitParameters(pth, bmean, bsigma, bgmean, bgsigma,
                                         bfitruns, maxr);
...
int GetProjectionTParameters(TH2D *histo2d, double *mean, double *sigma,
                             double *gmean, double *gsigma, double *runs) {
  TH1D *proj;
  ///    TH1D* proj2;
  int nbins = histo2d->GetNbinsX();
...
void DrawXsliceFitsFrom(TH2D *pthc, char *txt1, char *txt2, int timef,
                        double *mean, double *sigma, double *times) {
...
void DerivateArray(double *arr, double *sarr, int apoints, double dt, int np,
                   int neg = 1) {
  if (neg < 0)
    neg = -1;
  else
...
void DerivateArrayT(double *arr, double *sarr, int apoints, double dt,
                    double DT, int neg = 1) {
  if (neg < 0)
    neg = -1;
  else
...
double FindFraction(TSpline *spline, double zero, double min, double ampl,
                    double fraction) {
...
void FindTimesS(TGraph *graph, DPARAM *par) {
  ...
  /// Find start of pulse
  int zero = max;
  while (data[zero] > 0. && zero > 10)
    zero--;
  par->t1 = T[zero];
  //    cout <<" zero crossing at: "<<zero<<endl;
  /// Find end of Pulse
  int zero2 = max;
  while (data[zero2] > 0. && zero2 < points)
    zero2++;
  ...
  ///    cout<<"T(50%) = "<<par[2] <<" ampl = "<<ampl<<" tmax = "<< par[0]<<"
  ///    spline tmax = "<< par[3]<<"   spline t20 = "<< par[4]<<endl;
  delete spline;
  delete graphS;
  ///   delete graph;
}

void FindTimesSD(TGraph *graph, DPARAM *par, double tstart, double tend,
                 double *tf) {
  double *data = graph->GetY();
  double *T = graph->GetX();
  double dt = T[1] - T[0];
  ...
  /// Find start of pulse
  int istart = max;
  while (data[istart] > par->rms && istart > 10)
    istart--;
  par->t1 = T[istart];

  /// find end of pulse
  int iend = max;
  while (data[iend] > par->rms && iend < points)
    iend++;
  par->t2 = T[iend];
  ...
  if (fabs(smax - par->ampl) / (smax + par->ampl) > 0.03)
    smax = par->ampl;
...
double IntegratePulse(int npoints, double *data, double *integral, double dt,
                      double tint) {
  //   dt*=1e9;
...
int AnalyseIntegratedPulse(int points, double *data, IPARAM *par,
                           double threshold, double dt) {
  /// use the integrated+filtered pulse to define a region where a trigger
  /// occured. (integral above threshold)
  ...
  for (int i = par->t10; i > 0; i--) {
    if (i < tpoint)
      par->charge += data[i];
    par->stime = i;
    if (data[i] > threshold / 100. ||
        (data[i + 1] - data[i - 1]) / dt >= -0.0001) {
      break;
    }
  }
...
int SubtractRefferenceChannel(int *spoints, double **data, double *sampl,
                              int cref, double *bsl) {
  ...
  for (int i = 0; i < points; i++) {
    sampl[i] = f[0] * (data[0][i] - bsl[0]) + f[1] * (data[1][i] - bsl[1]) +
               f[2] * (data[2][i] - bsl[2]) + f[3] * (data[3][i] - bsl[3]);
  }

  return (-f[cref]);
}

int SubtractMaxChannel(int points, double **data, double *sampl, double *bsl,
                       int *mask) {
  if (points <= 0) {
...
int SubtractTwoChannels(int points, double **data, double *sampl, double *bsl,
                        int *mask) {
  if (points <= 0) {
    cout << "Nothing to subtract!!!" << endl;
    return -1;
...
int SubtractBaselineOld(int points, double **data, double *sampl, double *bsl,
                        int *mask) {
  if (points <= 0) {
    cout << "Nothing to subtract!!!" << endl;
    return -1;
...
int AnalyseLongPulse(int points, double *data, double *drv, IPARAM *par,
                     double threshold, double dt, int tshift) {
  /// use the integrated+filtered pulse to define a region where a trigger
  /// occured. (integral above threshold)
  /// pulses are considered negative!!!
  if (points - tshift < 20)
  ...
  if (ntrig <= 0)
    return (-1); // cout<<"No trigger in event!"<<endl;
  ...
  for (int i = tpoint; i < points; i++) {
    ...
    if (data[i] <= threshold) {
      par->tot++;
      par->ftime = i; /// this is added to avoid a pulse at the end of the data
                      /// that does not return to 0!!!
  ...
  for (int i = par->t90; i > 0; i--) {
    par->risecharge += data[i];
    if (data[i] >= par->ampl * 0.1 ||
        (data[i] > 0.5 * threshold && fabs(drv[i]) <= drvtrig)) {
      par->t10 = i;
      break;
    }
  }
  ...
  for (int i = par->t10; i > 0; i--) {
    ...
    //       par->charge+=data[i];
    //     }
    if (data[i] > threshold / 5. ||
        (fabs(drv[i]) <= drvtrig && data[i] > threshold * 0.8)) {
      par->stime = i;
      break;
    }
  }
  ...
  par->bslch = -0.5 * (data[par->stime] + data[par->ftime]) *
               (par->ftime - par->stime + 1.);
  par->width = par->ftime - par->stime;
  par->ampl *= -1.;
...
void DivideHspark(TH1D *h1, TH1D *h2, TH1D *hspark) {
  ...
  if (npulses == 0) {
    cout << " ATTENTION !!!! Time binwidth smaller than LINAC4 periode "
            "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
         << endl;
    npulses = 1;
  }
  ...
  int n2 = h2->GetNbinsX();
  if (n1 != n2) {
    cout << "ATTENTION !!!!!!   Plot and normalization plot do not have the "
            "same number of bins!!!!!!!"
         << endl;
    if (n1 > n2)
      n1 = n2;
  ...
  for (int i = 0; i <= n1; i++) {
    ...
    if (y + nsparks > npulses) {
      cout << " ******     found bin with " << y << " entries and " << nsparks
           << " sparks & recoveries    *******" << endl;
      npulses = y;
    }
    y = npulses;
    sx = sqrt(x);
