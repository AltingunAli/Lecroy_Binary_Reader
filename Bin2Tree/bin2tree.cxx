...
int main(int argc, char *argv[]){
  ...
  if ((detNo>=RUNMIN && detNo<=RUNMAX) || (detNo>=RUNMIN2 && detNo<=RUNMAX2)|| (detNo>=RUNMIN3 && detNo<=RUNMAX3))
    {
      ...
      if (ftmp == NULL)
	{
      ...
      snprintf(command, sizeof(command), "cd %s\nls -d S%04d-%02d*/ > %s", basedatapath, detNo, runNo, afile);
      else 
      snprintf(command, sizeof(command), "cd %s\nls -d S%06d-%02d*/ > %s", basedatapath, detNo, runNo, afile);
      ...
      if (fgets(fnametmp,200,ftmp) == NULL)
	{
      ...
      //       int stst = sscanf(fnametmp,"S%03d-%d-%d-%3f-%3f%30s",&rtmp,&vm,&vd,&bthick,&dgap,ftypetmp);
      cout <<"___________________________________________________\n\narguments read = "<<stst<<endl;
      filetype.assign(ftypetmp);
      cout<<"detector = "<<dtmp<<" (S"<< std::setfill('0') << std::setw(width)<<detNo<<")"<<endl;
      cout<<"run = "<<rtmp<<endl;
      cout<<"Vm = "<<vm<<endl;
      cout<<"Vd = "<<vd<<endl;
...
//       cout<<"Drift gap = "<<dgap<<endl;
  ...
  int runb = 0;
  snprintf(dirname, sizeof(dirname), "%s/S%03d-%02d-%d-%d", basedatapath, detNo, runNo, vm, vd);
  if (detNo>100)
    snprintf(dirname, sizeof(dirname), "%s/S%04d-%02d-%d-%d", basedatapath, detNo, runNo, vm, vd);
  if (detNo>10000)
    snprintf(dirname, sizeof(dirname), "%s/S%06d-%02d-%d-%d", basedatapath, detNo, runNo, vm, vd);
...
//     cout <<endl;
    ...
    snprintf(ofname, sizeof(ofname), "%s/S%03d-%02d-%d-%d", basedirname, detNo, runNo, vm, vd);
    if (detNo>100)
      snprintf(ofname, sizeof(ofname), "%s/S%04d-%02d-%d-%d", basedirname, detNo, runNo, vm, vd);
    if (detNo>10000)
      snprintf(ofname, sizeof(ofname), "%s/S%06d-%02d-%d-%d", basedirname, detNo, runNo, vm, vd);
    if (runb)
...
//     tree.Branch("c3ampl", c3, "c3ampl[npoints]/D");
	...
	for (int ci=0;ci<0;ci++)
	{
	  ...
	  snprintf(fnametmp, sizeof(fnametmp), "%s/C%d%s%05d.trc", dirname, ci+1, namebase, ev);
	...
	if (active[1-1])
	{
	  snprintf(fileName, sizeof(fileName), "%s/C%d%s%05d.trc", dirname, 1, namebase, ev);
  ...
  // 	printWavedesc(desc1);
	...
	if (active[2-1])
	{
	  snprintf(fileName, sizeof(fileName), "%s/C%d%s%05d.trc", dirname, 2, namebase, ev);
// 	  fprintf(stderr, "file: %s\n", fileName);
	  if((file = fopen(fileName, "r")) == NULL)  {
          cout<<"can not open file "<<fileName<<endl;
          continue; //return 1;
  ...
  // 	printWavedesc(desc2);
	...
	if (active[3-1])
	{
	  snprintf(fileName, sizeof(fileName), "%s/C%d%s%05d.trc", dirname, 3, namebase, ev);
	  //fprintf(stderr, "file: %s\n", fileName);
	  if((file = fopen(fileName, "r")) == NULL)
      {
          cout<<"can not open file "<<fileName<<endl;
...
// 	  cout << "FULL RANGE = "<<(desc3.max_value)*gain[2]-offset[2] - (desc3.min_value)*gain[2]-offset[2] <<" = "<<(desc3.max_value*gain[2]-offset[2] - desc3.min_value*gain[2]-offset[2])/8 <<" per div"<<endl;
	...
	if (active[4-1])
	{
	  snprintf(fileName, sizeof(fileName), "%s/C%d%s%05d.trc", dirname, 4, namebase, ev);
	  //fprintf(stderr, "file: %s\n", fileName);
	  if((file = fopen(fileName, "r")) == NULL)
      {
          cout<<"can not open file "<<fileName<<endl;
	...
	for(int ii = 0; ii < II; ii++)
	  {
	    ...
	    switch (tchan) {
	      case 0: snprintf(date, sizeof(date), "%04d/%02d/%02d_%02d:%02d:%02d", (int)desc1.trigger_time_Y, (int)desc1.trigger_time_M, (int)desc1.trigger_time_D, (int)desc1.trigger_time_h, (int)desc1.trigger_time_m, (int)desc1.trigger_time_s); 
		      dt = desc1.horizontal_interval;
		      sec = modf(desc1.trigger_time_s,&intpart) + desc1.horizontal_offset;
		      break;
	      case 1: snprintf(date, sizeof(date), "%04d/%02d/%02d_%02d:%02d:%02d", (int)desc2.trigger_time_Y, (int)desc2.trigger_time_M, (int)desc2.trigger_time_D, (int)desc2.trigger_time_h, (int)desc2.trigger_time_m, (int)desc2.trigger_time_s); 
		      dt = desc2.horizontal_interval;
		      sec = modf(desc2.trigger_time_s,&intpart) + desc2.horizontal_offset;
		      break;
	      case 2: snprintf(date, sizeof(date), "%04d/%02d/%02d_%02d:%02d:%02d", (int)desc3.trigger_time_Y, (int)desc3.trigger_time_M, (int)desc3.trigger_time_D, (int)desc3.trigger_time_h, (int)desc3.trigger_time_m, (int)desc3.trigger_time_s);
		      dt = desc3.horizontal_interval;
		      sec = modf(desc3.trigger_time_s,&intpart) + desc3.horizontal_offset;
		      break;
	      case 3: snprintf(date, sizeof(date), "%04d/%02d/%02d_%02d:%02d:%02d", (int)desc4.trigger_time_Y, (int)desc4.trigger_time_M, (int)desc4.trigger_time_D, (int)desc4.trigger_time_h, (int)desc4.trigger_time_m, (int)desc4.trigger_time_s);
...
// //       cin>>tstt;
    ...
    snprintf(command, sizeof(command), "cd %s/bin\n ./makeTree %d %d \n", WORKDIR, detNo, runNo);
    if (detNo>=10000)
    {
      snprintf(command, sizeof(command), "cd %s/bin\n ./makeTreeMulti %d %d 1 \n", WORKDIR, detNo, runNo);
      cout<<"Executing "<<command<<endl;
      tstt = system(command);
      cout<<"Creation of processed data tree 1 has started! Will sleep for 5 sec before continuing!"<<endl;
      for (int i=0;i<5;i++)
      {
      ...
      cout<<"Executing "<<command<<endl;
      tstt = system(command);
      cout<<"Creation of processed data tree 2 has started! Will sleep for 5 sec before exiting!"<<endl;
      for (int i=0;i<5;i++)
      {
         cout<<"\rexit in "<<5-i<<"... "<<flush;
         gSystem->Sleep(1000);
      ...
      cout<<"Executing "<<command<<endl;
      tstt = system(command);
      cout<<"Creation of processed data tree 3 has started! Will sleep for 5 sec before exiting!"<<endl;
      for (int i=0;i<5;i++)
      {
         cout<<"\rexit in "<<5-i<<"... "<<flush;
         gSystem->Sleep(1000);
    ...
    else
    {
        ...
        cout<<"Executing "<<command<<endl;
        tstt = system(command);
        cout<<"Creation of processed data tree has started! Will sleep for 10 sec before exiting!"<<endl;
        for (int i=0;i<10;i++)
        {
