...
int main(int argc, char **argv)
{
  ...
  char fname[100];
  snprintf(fname,sizeof(fname),"%s/logs/analysis/logS%03d-%02d.txt",WORKDIR,detid,runid);
  FILE *ftmp = fopen(fname,"w");
  fclose(ftmp);
  char command[2000];
  snprintf(command, sizeof(command), "root -b -x '%s/AnalyseTreeProduction.C+(%d,%d)' -q >> %s &\n", WORKDIR, detid, runid, fname);
  cout<<"executing:\n"<<command<<endl;
  system(command);
