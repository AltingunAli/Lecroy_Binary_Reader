...
int main(int argc, char **argv) {
  ...
  if (argc > 3) {
    ...
    if (tst <= 0 || tst2 <= 0 || tst3 <= 0) {
      cout << "syntax: makeTree <detector number> <run number> <channel number>"
           << endl;
      return -1;
    }
  } else {
  ...
  char fname[100];
  snprintf(fname, 100, "%s/logs/logS%03d-%02d-ch%d.txt", WORKDIR, detid, runid,
           line);
  FILE *ftmp = fopen(fname, "w");
  fclose(ftmp);
  char command[2000];
  snprintf(command, 2000,
           "root -b -x '%s/MakeTreefromRawTreeProductionMulti.C+(%d,%d,%d)' -q "
           ">> %s/logs/logS%03d-%02d-ch%d.txt &\n",
