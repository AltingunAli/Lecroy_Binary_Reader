...
int MakeTreefromRawTreeProductionDeb(int detNo=0, int runNo=1, int trigger=0, string filetype = "")  ///put trigger = ChannelNo in case of external trigger
{
  ...
  if (detNo>=MINRUN && detNo<=MAXRUN)
  {
    ...
    fclose(ftmp);
    sprintf(command,"cd %s\nls -d S%03d-%02d-*%s_%s*.root > %s 2>/dev/null",basedirname,detNo,runNo,filetype.c_str(),rtype,afile);
...
// //     TString bname = "npoints"+channel;
// //     TString btype = bname+"/I";
// // //     outtree->Branch(bname, &spoints[i], btype);   
// //     bname = "amplC"+channel;
// //     btype = bname+"[npoints"+channel+"]/D";
...
//   outtree->Branch("amplSum2", amplSum[1], "amplSum2[sumpoints]/D");
  ...
  TF1* fbsl = new TF1(fname2,"gaus",-2.,2.);
  fbsl->SetNpx(2000);
  sprintf(fname2,"gaus0");
  TF1* fbsl0 = new TF1(fname2,"gaus",-2.,2.);
  fbsl0->SetNpx(2000);

  sprintf(fname2,"gausCtot");
  TF1* fbslall = new TF1(fname2,"gaus",-2.,2.);
  fbslall->SetNpx(2000);
  sprintf(fname2,"gaus0tot");
  TF1* fbsl0all = new TF1(fname2,"gaus",-2.,2.);
...
#ifdef SLOWFILES
      cout<<endl<<endl<<"ATTENTION !!!\n The data processing is done with SLOWFILES flag on!!!\n This means that the baseline is subtracted only every 500 events!!!"<<endl<<endl<<endl;  
#endif
  double tdt = -1.;  
...
//        cout<<"Event "<<evNo;
//         cout<< " idt = " << idt << "\t epoch = " << epoch << "\t inn= " << inn << "\t nn= " << nn << endl;
...
//  if (nEv==0)	  
	...
	{
	  //cout << "amplC[ci][k]= " << amplC[ci][k] << endl;
	  if(amplC[trgch][k]>1.) 
	  {
	    itrigger =k;
      ...
      ttrig = itrigger * dt;
//        cout<<"Event "<<evNo;
//         cout<< " itrigger = " << itrigger << "\t ttrig = " << ttrig << endl;
//       continue;
...
/// For this reason we check the sumsig. If it is 0 or very close to 0, reject completely the event!!!!
	...
	fitstatus1 = hbsl->Fit(fbsl,"BQ","",meant-3*rmst,meant+3*rmst);
// cout<< fitstatus1<<" ";
//              hbsl->Fit(fbsl,"MBRQ","",meant-0.01,meant+0.01);
	...
	ngpoints++;
      //hbsl[ci]->GetXaxis()->SetRangeUser(floor(bsl[ci]*100.+0.5)/100-0.02,floor(bsl[ci]*100.+0.5)/100+0.02);
// 	hbsl->GetXaxis()->SetRangeUser(0.01,0.03);
	cbsl->Modified();
	cbsl->Update();

	hbslall->Add(hbsl);
	cout<<"done "<<nEv<<"  "<<evNo<<endl;
...
/// fit and draw the acumulated baseline of the signals
	...
	hbslall->Fit(fbslall,"BQ","",bsl[1]-0.01,bsl[1]+0.01);
// 	hbslall->GetXaxis()->SetRangeUser(-0.01,0.03);
	hbslall->Draw("E0");
	bsl[1]=fbslall->GetParameter(1);
  //	hbsl[1]->GetXaxis()->SetRangeUser(-0.02,0.02);
	cbsl->Modified();
	cbsl->Update();
